...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public boolean rentPlant(int PlantId, String renter){
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(RENTED_ITEM, PlantId);
        values.put(RENTER, renter);
        long id = db.insert(RENTAL_ORDERS, null, values);
        Cursor cursor = db.rawQuery("Select * from " + RENTAL_ORDERS, new String[]{});
        //System.out.println("You rent a plant successfully");
        //System.out.println("Your rent id is " + id);
        db.close();
        return true;
    }
